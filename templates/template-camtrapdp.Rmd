---
title: "Map data to Camtrap DP 1.0.2"
...
This script serves as a template for mapping raw data Camera Trap Data Package (Camtrap DP) 1.0.2.
...
Check https://camtrap-dp.tdwg.org/ for the specifications of Camtrap DP, and https://inbo.github.io/camtrapdp/ for the R package to work with Camtrap DP in R.
...
Probably the data will need to be pre-processed before it can be mapped to Camtrap DP. This can include cleaning, filtering, or transforming the data to fit the structure required by Camtrap DP.
...
Create the three resources (deployments, media, observations) based on the pre-proccessed data.
...
Start with compiling the three resources into a frictionless data package, adding metadata such as schema files and project information.
...
package <-
  ...
  append(c(
    name = NA_character_, # Required
    profile = "https://raw.githubusercontent.com/tdwg/camtrap-dp/1.0.2/camtrap-dp-profile.json", # Required
    id = NULL,
...
```{r}
# `here` starts at the current working directory at the time when the package is loaded
frictionless::write_package(package, here::here("datasets", datasetname_name, "data-interim"))
```

## Read with camtrapdp

Now, read the Camtrap DP data package to automatically update the spatial, temporal, and taxonomic coverage fields in the metadata based on the resources. See the function reference of [`camtrapdp::read_camtrapdp()`](https://inbo.github.io/camtrapdp/reference/read_camtrapdp.html).
